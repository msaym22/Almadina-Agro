...
const InvoiceGenerator = ({ invoiceData }) => {
  ...
  const getProductNameForLanguage = (item) => {
    // item.productName contains the full English name (e.g., "Berring UC 205")
    // item.productNameUrdu contains just the Urdu part (e.g., "برنگ")
    ...
    // --- DIAGNOSTIC LOG ---
    console.log("InvoiceData for rendering (JSX):", invoiceData);
    // --- END DIAGNOSTIC LOG ---
